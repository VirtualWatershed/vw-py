...
def execute(directory,command):
    ...
    #devnull = None
    process = subprocess.Popen(command, stdout=devnull,stderr=devnull,cwd=directory)
    ...
    while i < len(lines):
        if CONTROL_SEPERATOR==lines[i]:
            try:
                ...
                raise Exception('control file ill formatted.Possible problem at line {0}'.format(i+1))
        else:
            raise Exception('control file ill formatted.Possible problem at line {0}'.format(i+1))
        i = i + 4 + num_vars
...
def create_control(control_data,output_path):
    ...
    with open(output_path,'w') as f:
        ...
        for variable in sorted(data.keys()):
            ...
            f.write(str(CONTROL_VAR_TYPES_INV[data[variable]['datatype']])+'\n')
...
def run_prms(prmsdir=None,data_in=None,param_in=None,control_in=None,event_emitter=None,*args,**kwargs):
    ...
    control_vars['data']['stats_output_file']['values'] = ['./output/statvar.dat']
    control_vars['data']['ani_output_file']['values'] = ['./output/animation.out']
    control_vars['data']['csv_output_file']['values'] = ['./output/gsflow.csv']
    control_vars['data']['model_output_file']['values'] = ['./output/prms.out']
    control_vars['data']['gsflow_output_file']['values'] = ['./output/gsflow.out']
    ...
    # coution: prms adds nhru extension at the end of animation file name automatically
    output_locs = {
...
def prms(data_path=None,param_path=None,control_path=None,output_path=None,animation_path=None,statsvar_path=None,event_emitter=None,*args,**kwargs):
    ...
    netcdf_to_parameter(param_path,param_in,event_emitter=event_emitter,**kwargs)
