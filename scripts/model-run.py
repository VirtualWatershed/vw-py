...
#get user input
hostname = input('Hostname or IP Address: ')
u = input('Username: ')
p = getpass.getpass()
parent_model_run_uuid = input('Enter the model_run_uuid that you want to use as a source: ')
...
SRC_DATASET_URL = "https://"+ hostname + "/apps/my_app/search/datasets.json?version=3&model_run_uuid=" + parent_model_run_uuid + "&limit=2005"
...
json_template="isnobal.json"

xml_template="isnobal_template.xml"
...
def fetchInputData(parent_model_run_uuid):
    ...
    if status.lower() in ['true']:
        ...
        for i in data["results"]:
            ...
            urllib.request.urlretrieve(full, outfile)
    else:
        print(" ")
    ...
    for q in dir_list:
        ...
        if(os.path.isfile(source_filepath)):
            ...
            if(os.path.isfile(source_filepath)):
                ...
                template=Template(template_object.read())
                output = template.substitute(wcs=wcs,wms=wms,tax=tax,ext=ext,mimetype=mimetype,orig_epsg=orig_epsg,westBnd=westBnd,eastBnd=eastBnd,northBnd=northBnd,southBnd=southBnd,epsg=epsg,model_run_uuid=model_run_uuid,description=description,basename=basename,recs=recs,features=features,modelname=modelname,location=location,state=state,inputFilePath=inputFilePath,xmlFilePath=xmlFilePath,model_set=model_set,model_set_type=model_set_type,model_set_taxonomy=model_set_taxonomy)
            ...
            for json_file in json_files:
                ...
                post_data['metadata']['xml'] = etree.tostring(xml, encoding=str)
                apps = post_data['apps']
                if len(apps) > 1:
